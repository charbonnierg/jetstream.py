...
#   filename:  client_disconnect.json
...
class Sent(BaseModel):
    msgs: Optional[int] = Field(
        None, description="The number of messages handled by the client"
    )
    bytes: Optional[int] = Field(
        None, description="The number of bytes handled by the client"
    )
...
class IoNatsServerAdvisoryV1ClientDisconnect(BaseModel):
    type: str
    ...
    sent: Sent = Field(..., description="Data sent to the client")
    received: Received = Field(..., description="Data received from the client")
    reason: str = Field(..., description="The reason the client disconnected")
